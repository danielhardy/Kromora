...
enum PhotosAuthorizationState: String, Codable, Sendable, Equatable {
    ...
    var recoveryMessage: String? {
        ...
        case .authorized:
            return nil
        case .notDetermined:
            return "Allow Kromora to add photos when macOS asks for Photos access, then retry."
        case .denied:
            return "Photos access is denied. Enable Kromora in System Settings > Privacy & Security > Photos, then retry."
        case .restricted:
            return "Photos access is restricted by macOS or device policy and cannot be changed in Kromora."
        case .limited:
