...
/// Where a Look came from. Bundled Looks are packaged with Kromora and are immutable; all other
...
struct CubeLUT: Identifiable, Hashable, Sendable {
    ...
    init(
        ...
        while let rawLine = try reader.readLine() {
            ...
            if first == "TITLE" {
                // TITLE is display metadata. Kromora keeps the filename as the stable, predictable
    ...
    static func cubeFileContents(
        ...
        precondition(cube.count == size * size * size, "cube count must equal size^3")
        var lines: [String] = []
        lines.reserveCapacity(cube.count + comments.count + 6)
        lines.append("# Generated by Kromora")
        for comment in comments {
            lines.append(contentsOf: comment.components(separatedBy: .newlines).map {
