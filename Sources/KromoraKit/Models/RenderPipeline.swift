...
enum RenderPipeline {
    ...
    /// The display orientation recorded for a RAW source, read via ImageIO without
    /// developing pixels. Internal for the engine session, which needs the same answer
    /// without touching `outputImage`, and for tests. Falls back to `.up`.
    static func rawOrientation(for backing: ImageSource.Backing) -> CGImagePropertyOrientation {
        switch backing {
        case .url(let url):
            return ImageDecoder.exifOrientation(at: url)
        case .data(let data):
            return ImageDecoder.exifOrientation(of: data)
        }
    }
    ...
    static func grainSeed(for source: ImageSource) -> UInt32 {
        let digest = RenderCacheHash.digest(Data("Kromora.grain.v1:\(source.cacheFingerprint)".utf8))
