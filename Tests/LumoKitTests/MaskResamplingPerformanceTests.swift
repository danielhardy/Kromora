...
/// with `LUMO_MASK_RESAMPLING_BENCHMARK=1`; the ticket records the resulting wall times alongside
/// the scalar-loop baseline.
final class MaskResamplingPerformanceTests: XCTestCase {
    func testPlanarMaskUpscaleBenchmark() throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["LUMO_MASK_RESAMPLING_BENCHMARK"] == "1",
            "set LUMO_MASK_RESAMPLING_BENCHMARK=1 to run the mask resize timing harness"
        )
        ...
        let iterations = max(
            3, Int(ProcessInfo.processInfo.environment["LUMO_MASK_RESAMPLING_ITERATIONS"] ?? "5") ?? 5
        )
        ...
        let configuration = ProcessInfo.processInfo.environment[
            "LUMO_MASK_RESAMPLING_CONFIGURATION"
        ] ?? "unspecified"
        print(
            String(
