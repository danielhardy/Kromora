...
/// `KROMORA_PHOTO_ANALYSIS_BENCHMARK=1` and records the printed medians in the LUMO-206 completion
...
final class PhotoAnalysisPerformanceTests: TempDirectoryTestCase {
    ...
    private var iterations: Int {
        max(
            3,
            Int(ProcessInfo.processInfo.environment["KROMORA_PHOTO_ANALYSIS_ITERATIONS"] ?? "5") ?? 5)
    }
    ...
    private func requireBenchmark() throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["KROMORA_PHOTO_ANALYSIS_BENCHMARK"] == "1",
            "set KROMORA_PHOTO_ANALYSIS_BENCHMARK=1 to run the Release photo-analysis benchmark suite"
        )
    }
