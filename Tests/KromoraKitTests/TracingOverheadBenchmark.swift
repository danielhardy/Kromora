...
final class TracingOverheadBenchmark: XCTestCase {
    func testMeasureTracingOverhead() throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["KROMORA_TRACE_BENCHMARK"] != nil,
            "set KROMORA_TRACE_BENCHMARK=1 to measure tracing overhead"
        )
        let source = ImageSource(url: URL(fileURLWithPath: "/tmp/tracing-benchmark.png"),
                                 nativeExtent: CGSize(width: 1600, height: 1200))
        let iterations = 50_000
        let context = KromoraTraceContext(source: source, quality: .interactive)
        let warmup = 1_000

        for _ in 0..<warmup {
            KromoraObservability.event(.pointerInput, source: source, quality: .interactive,
                                    detail: "revision=1")
        }
