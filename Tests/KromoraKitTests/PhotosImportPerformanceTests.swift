...
final class PhotosImportPerformanceTests: TempDirectoryTestCase {
    ...
    func testProfileOneLargeRAWImportBeforeAndAfter() throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["KROMORA_PHOTOS_BENCHMARK"] != nil,
            "set KROMORA_PHOTOS_BENCHMARK=1 to run the Photos import timing harness"
        )
        guard let path = ProcessInfo.processInfo.environment["KROMORA_PHOTOS_RAW"] else {
            throw XCTSkip("set KROMORA_PHOTOS_RAW to a representative 40–60 MP RAW")
        }
        let url = URL(fileURLWithPath: path)
        guard let sourceData = try? Data(contentsOf: url) else {
            throw XCTSkip("could not read KROMORA_PHOTOS_RAW")
        }
