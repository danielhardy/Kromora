...
final class RenderEngineTests: TempDirectoryTestCase {
    ...
    func testExportMetadataPolicyRoundTripsSourceMetadataForEveryFormat() async throws {
        let url = try Fixtures.writeJPEG(
            ...
            in: tempDirectory,
            exif: [
                kCGImagePropertyExifLensModel: "Kromora Prime 35mm",
                kCGImagePropertyExifDateTimeOriginal: "2026:09:02 12:34:56",
            ],
            tiff: [
                kCGImagePropertyTIFFMake: "Kromora",
                kCGImagePropertyTIFFModel: "Test Body",
                kCGImagePropertyTIFFSoftware: "Kromora Tests",
            ],
            gps: [
        ...
        for format in ExportFormat.allCases {
            ...
            XCTAssertEqual(exif[kCGImagePropertyExifLensModel] as? String, "Kromora Prime 35mm", format.rawValue)
            XCTAssertEqual(exif[kCGImagePropertyExifDateTimeOriginal] as? String, "2026:09:02 12:34:56", format.rawValue)
            XCTAssertEqual(tiff[kCGImagePropertyTIFFMake] as? String, "Kromora", format.rawValue)
            XCTAssertEqual(tiff[kCGImagePropertyTIFFModel] as? String, "Test Body", format.rawValue)
            XCTAssertEqual(tiff[kCGImagePropertyTIFFSoftware] as? String, "Kromora Tests", format.rawValue)
    ...
    func testLocationPolicyIsExplicitForURLAndDataSourcesAcrossShareableFormats() async throws {
        let url = try Fixtures.writeJPEG(
            ...
            in: tempDirectory,
            exif: [kCGImagePropertyExifLensModel: "Kromora Prime 35mm"],
            tiff: [kCGImagePropertyTIFFMake: "Kromora"],
            gps: [
                kCGImagePropertyGPSLatitude: 43.6150,
                kCGImagePropertyGPSLongitude: 116.2023,
            ]
    ...
    func testStripMetadataDoesNotCopySourcePhotographicDictionaries() async throws {
        let url = try Fixtures.writeJPEG(
            ...
            in: tempDirectory,
            exif: [kCGImagePropertyExifLensModel: "Kromora Prime 35mm"],
            tiff: [kCGImagePropertyTIFFMake: "Kromora", kCGImagePropertyTIFFModel: "Test Body"],
