...
final class MetalPresentationBenchmark: XCTestCase {
    ...
    func testRealMetalPresentationBenchmark() async throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["KROMORA_METAL_BENCHMARK"] != nil,
            "set KROMORA_METAL_BENCHMARK=1 on a logged-in macOS display to run the hardware benchmark"
        )
        ...
        let sourcePath = ProcessInfo.processInfo.environment["KROMORA_METAL_BENCHMARK_RAW"]
        ...
        if let sourcePath {
            ...
            guard FileManager.default.fileExists(atPath: sourceURL.path) else {
                throw XCTSkip("KROMORA_METAL_BENCHMARK_RAW does not exist: \(sourcePath)")
            }
        ...
        let count = Int(ProcessInfo.processInfo.environment["KROMORA_METAL_BENCHMARK_ITERATIONS"] ?? "20") ?? 20
    ...
    func testRealMetalVignettePresentationRegression() async throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["KROMORA_METAL_BENCHMARK"] != nil,
            "set KROMORA_METAL_BENCHMARK=1 on a logged-in macOS display to run the hardware regression"
        )
        guard let device = MTLCreateSystemDefaultDevice(),
              let queue = device.makeCommandQueue() else {
            throw XCTSkip("Metal is unavailable")
