...
final class PreviewCostBenchmark: XCTestCase {
    ...
    func testMeasurePerRenderPreviewCost() throws {
        try XCTSkipUnless(
            ...
            "set KROMORA_BENCH=1 to run the preview-cost measurement"
        )
    ...
    func testMeasureToneCurveDragCost() throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["KROMORA_BENCH"] != nil,
            "set KROMORA_BENCH=1 to run the tone-curve measurement"
        )
    ...
    func testMeasureEffectsInteractiveCost() throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["KROMORA_BENCH"] != nil,
            "set KROMORA_BENCH=1 to run the Effects interactive-cost measurement"
        )
    ...
    ///
    /// `swift test --filter PreviewCostBenchmark` with `KROMORA_BENCH=1`.
    func testMeasureExportCost() throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["KROMORA_BENCH"] != nil,
            "set KROMORA_BENCH=1 to run the export-cost measurement"
        )
