...
final class ObservabilityTests: XCTestCase {
    ...
    func testSourceTokensAreStablePrivateSafeAndDistinct() {
        ...
        let first = KromoraTraceContext(sourceFingerprint: privatePath, quality: "preview")
        let second = KromoraTraceContext(sourceFingerprint: privatePath, quality: "preview")
        let different = KromoraTraceContext(
    ...
    func testEndingAnIntervalMoreThanOnceIsSafe() {
        var interval = KromoraSignpostInterval(.render, context: .unknown)
        interval.end()
        interval.end()
    }
