...
final class MaskOverlayPerformanceBenchmark: XCTestCase {
    ...
    func testRealMaskOverlayPresentationBenchmark() async throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["KROMORA_MASK_OVERLAY_BENCHMARK"] != nil,
            "set KROMORA_MASK_OVERLAY_BENCHMARK=1 on a logged-in macOS display to run the hardware benchmark"
        )
        ...
        let iterations = max(10, min(
            Int(ProcessInfo.processInfo.environment["KROMORA_MASK_OVERLAY_ITERATIONS"] ?? "30") ?? 30,
            200
        ))
