...
/// `KROMORA_CONCURRENT_CAPTURE=1` is set on a logged-in macOS display.
@MainActor
final class ConcurrentExportEditingBenchmark: XCTestCase {
    ...
    func testRealConcurrentBatchExportAndEditing() async throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["KROMORA_CONCURRENT_CAPTURE"] != nil,
            "set KROMORA_CONCURRENT_CAPTURE=1 on a logged-in macOS display to run the concurrent capture"
        )
        ...
        let itemCount = max(1, Int(environment["KROMORA_CONCURRENT_CAPTURE_ITEMS"] ?? "") ?? 6)
        let gestureCount = max(1, Int(environment["KROMORA_CONCURRENT_CAPTURE_GESTURES"] ?? "") ?? 10)
        let framesPerGesture = max(1, Int(environment["KROMORA_CONCURRENT_CAPTURE_FRAMES"] ?? "") ?? 6)
        ...
        let primaryPath = environment["KROMORA_CONCURRENT_CAPTURE_RAW"]
        ...
        let secondaryURL = environment["KROMORA_CONCURRENT_CAPTURE_RAW_SECONDARY"].map(URL.init(fileURLWithPath:))
