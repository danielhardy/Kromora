...
Inspect `KromoraIcon.svg` at 1024 px and the generated 16, 32, and 128 px PNGs.
...
The build script writes only to `.build/Kromora.app` and `.build/kromora-icon-asset-
validation`; both are generated build outputs. The checked-in
`Sources/Kromora/Info.plist` sets `CFBundleIconName=AppIcon`, and `actool` compiles the catalog into
