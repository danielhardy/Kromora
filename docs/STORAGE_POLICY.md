...
| SwiftData edit projection | `EditDocumentStore` in package mode | In memory; no production standalone edit database is authoritative | Disposable projection updated after the package commit | Not required; edits are restored from package sidecars |
