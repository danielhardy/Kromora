# Kromora — code review and guardrail audit

_Historical findings reconciled with the current Kromora package, CI, and `PHASE2_SPEC.md` on September
3, 2026._
...
retain their historical evidence while current status and names use Kromora's present architecture.
...
- ~~**No tests, anywhere.**~~ **[fixed]** — the package is split into `KromoraKit` plus a thin `@main`
...
- **`docs/PHASE2_SPEC.md` is a living implementation record** rather than the original 4,180-line
  multi-agent output. It is kept reconciled with the current Kromora architecture and test inventory;
...
1. ~~**`KromoraKit` split + test target.**~~ **Done** — see §2.
...
- **`RecipeExtractor.derive` end-to-end is tested only where a RAW exists.** Phase 2 Step 9 added
  ...
  when `KROMORA_RAW_FIXTURE_DIR` is not supplied** — read the deterministic lane's green tick as saying
...
- **The `CIRAWFilter` half of `RAWDevelopSettings` only runs where a RAW exists.** Several tests build
  a real filter from `Fixtures.localRAWURL` — a file under `KROMORA_RAW_FIXTURE_DIR` — and `XCTSkip` when
