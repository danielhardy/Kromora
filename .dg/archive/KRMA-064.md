...
updated: 2026-09-10T12:53:37.854Z
order: tdjanm1o
board: product
...
KRMA-018 (verification child; see its comment thread for full context).
...
`Sources/LumoKit/Models/ImageCollection.swift` as KRMA-018's `PhotoAsset`-based `Item` rework.
...
- Manually confirm `ImageCollection` still produces stable `PhotoAssetID`s per KRMA-018's acceptance
