...
updated: 2026-09-10T12:53:39.551Z
depends_on:
  - KRMA-046
order: ha2voh7c
board: product
...
This appears to be a regression in or around the zoom/pan work delivered by KRMA-046. The failure
...
transient and preserve the render-quality/coalescing behavior introduced by KRMA-046.

### Comment — codex @ 2026-09-01T18:16:33.267Z

Implemented and committed as 48a9ae0. Hardened preview publication across fit, fill, explicit, scroll, pinch, pan, and comparison navigation: PreviewSurface now retains the last GPU-confirmed frame, rejects failed candidates without blanking the canvas, ignores stale completions, and reports actionable display errors. Added navigation-to-surface regression coverage. Verification: swift test — 533 passed, 26 expected skips; swift build -c release passed; git diff --check passed; dg validate passed with only the known runner-model and KRMA-044 context warnings.
...
- `dg validate` — OK (only the known runner-model and KRMA-044 context-completeness warnings).
...
- No changes touched `CanvasNavigation.swift` (KRMA-046 territory) — correctly out of scope for this localized fix.
