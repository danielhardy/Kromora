---
id: KRMA-102
title: "KRMA-049 follow-up: filmstrip loses arrow-key stepping while a thumbnail button holds keyboard focus"
type: task
status: done
priority: low
labels:
...
updated: 2026-09-10T12:53:40.020Z
depends_on:
  - KRMA-049
order: 6ha2vog9
board: product
...
KRMA-049 follow-up: filmstrip loses arrow-key stepping while a thumbnail button holds keyboard focus

## Context

KRMA-049 (commit 5e1a7e4) widened `KeyMonitorPolicy.controlOwnsKeyboard` from "text input only" to
...
- Confirmed KeyMonitorPolicy.controlOwnsKeyboard is unchanged (still "any NSControl") — the fix restores Left/Right locally via .onKeyPress on the focused button rather than narrowing the global policy, so the slider/picker deferral this widened check was added for in KRMA-049 is untouched. `[`/`]` remain deferred to the control while a thumbnail has focus; AC only requires one of the two key sets to work, satisfied by Left/Right.
