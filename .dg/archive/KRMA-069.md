...
updated: 2026-09-21T02:12:04.407Z
order: 4gpcdtn7
board: product
commits:
  - e01d5e7
...
about 205.5 ms per direct 1600 x 1200 render even after the PNG round-trip was removed by KRMA-063,
...
**Action taken (per the unresolved-blocker protocol):** filed **KRMA-073** (urgent, `verification`
label, parent KRMA-069) to implement the actual persistent GPU presentation surface and stop
publishing interactive frames through `AppViewModel`'s broad `@Published` state. KRMA-069 now
depends on KRMA-073. Moving this issue back to `review` rather than `done` — the sizing/pacing
...
Re-verified after KRMA-073 (persistent GPU presentation surface, done at f3bb800/14a1ca3) and
its follow-up KRMA-075 (redundant settled rasterization removed, aa08b38) landed on main. All
...
- `PreviewCoordinator.render` (`PreviewCoordinator.swift:208-231`) only calls
  ...
  including settled frames, the second rasterization pass KRMA-075 flagged is gone.
...
essentially always, post-KRMA-073), that branch is dead and the badges never render — a real
...
No further blockers. KRMA-073/KRMA-075 dependencies satisfied; moving to done.

## Agent log

<!-- Generated summaries only. Detailed activity lives in events.jsonl. -->

- 2026-09-01T01:15:41.711Z: Re-verified after KRMA-073/075 landed persistent GPU surface + settled-frame fix. Fixed a regression where comparison badges stopped rendering once the GPU surface owns display.
