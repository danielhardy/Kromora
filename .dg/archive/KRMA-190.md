...
updated: 2026-09-10T12:53:45.861Z
depends_on:
  - KRMA-187
  - KRMA-185
order: w97rxoww
...
Saliency (KRMA-188) says *where* attention likely is; foreground instance masking says *which
...
2. Implement `mask(for: .background, image:, quality:)` as the complement of the union of all
   detected foreground instances (use `MaskOperations.invert`/`union` from KRMA-186 rather than a
   separate ad-hoc computation).
3. Masks written through `MaskStore` (KRMA-185), keyed consistently.
