...
updated: 2026-09-21T02:11:06.688Z
order: o1q7hkcb
board: product
commits:
  - e839d4c
...
**Depends on:** KRMA-197
**Epic:** KRMA-181 — see original proposal §20
...
2. Populated from `PhotoAnalysis.regions` + `globalTone` + the primary-subject pick (KRMA-197).
3. Fields are `nil` when inputs aren't available (no face → face-relative fields nil) — never a
   fabricated value.
4. Deltas use the `perceptual` luminance variant (KRMA-182) — document why.
...
- Not the place for aesthetic judgments ("subject is too dark") — states a signed delta only.
  Judgment happens in KRMA-199/200.
