...
updated: 2026-09-10T12:53:41.630Z
order: 8co6wtwy
board: product
commits:
  - fe187c8
...
selection. KRMA-085 and KRMA-041 document the current user-facing and persistence contracts.
...
1. **Pre-existing flake, already tracked (non-blocking):** `CropWorkflowTests.testReenteringCropRequestsTheFullUncroppedStageAndRestoresOnExit` failed in 2 of 3 full-suite runs, passed in isolation 3/3. Unrelated to fe187c8 (file last touched by KRMA-124/KRMA-115; the commit touches only `AppViewModel.swift`, `LookInspectorView.swift`, `LUTWorkflowTests.swift`). Evidence appended to **KRMA-132** (existing backlog child of KRMA-124).
2. **Filed as backlog child KRMA-133 (parent KRMA-128, label `verification`):** a click on a row that fails to resolve is silent when the active photo has no Look or a resolved one — `refreshLUTResolutionStatus()` reports only on the document's own ID, and the scan-complete retry is manual. Narrow window (rows render only from resolvable scan results; parsed `CubeLUT`s persist in memory between rescans), polish not correctness — criterion 4 holds for the primary cases.
...
- Backlog child filed: KRMA-133 (`verification`, parent KRMA-128)
- Flake evidence appended: KRMA-132
...
- 2026-09-02T14:44:56.660Z: Verified by counterpoint review (pi): full checks pass (swift build, release build, 602-test suite modulo a pre-existing unrelated crop flake tracked in KRMA-132, git diff --check). All five acceptance criteria hold; regression coverage confirmed. Non-blocking polish filed as KRMA-133.
