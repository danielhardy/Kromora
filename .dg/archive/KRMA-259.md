...
updated: 2026-09-10T12:53:51.395Z
depends_on:
  - KRMA-250
order: a0
board: product
...
Follow-up to KRMA-250. `onDiskFileURL` returns the *requested* container URL
...
- When resolving the URL for display (whether in the view's `.task` or in the provider from
  KRMA-258), check `FileManager.default.fileExists(atPath:)` and surface "no database yet"
...
- [ ] In-memory fallback behavior from KRMA-250 is unchanged.
