...
The window is narrow (rows normally exist only for resolvable entries, and parsed `CubeLUT`s stay in memory between rescans), so this is polish, not a correctness bug — KRMA-128's acceptance criterion 4 is met for the primary cases.
