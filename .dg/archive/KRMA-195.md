...
updated: 2026-09-10T12:53:46.211Z
depends_on:
  - KRMA-187
  - KRMA-192
order: djanm69y
...
Masking UI (KRMA-201) wants individual masks, often at `.preview`/`.render` quality, without
...
1. `actor PhotoAnalysisCoordinator` exposing (at least) two entry points:
   ...
   KRMA-185) and only compute what's missing.
...
- Structure the stage list so it's a small internal registry, not a hardcoded call chain — every
  mask-provider ticket that already landed by the time this is implemented (KRMA-188/189/190/191,
