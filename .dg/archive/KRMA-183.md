...
updated: 2026-09-10T12:53:45.333Z
depends_on:
  - KRMA-182
order: k5rcyk2k
board: product
...
`AnalysisImage` only. `NormalizedMask` and everything mask-shaped now lives in KRMA-184 (`RegionMask`
...
2. `struct AnalysisImage: Sendable` — an opaque handle produced once per analysis run. Not
   `Codable` (only `PhotoAnalysis` results are cached, per KRMA-196). Internally it may wrap a
...
3. `struct AnalysisImageFactory` (or free function) producing one `AnalysisImage` at a configurable
   ...
   ```
   Leave `// TODO(KRMA-206): benchmark 512/768/1024` rather than hand-tuning now.
4. A documented, one-shot Vision-coordinate → `NormalizedRect`/`NormalizedPoint` conversion
   function — the **only** place permitted to know Vision's coordinate convention. KRMA-187 and
...
- The Vision-coordinate conversion function can be unit-tested now with synthetic Vision-shaped
  rects even though no real Vision adapter exists yet (that's KRMA-187).
