...
updated: 2026-09-10T12:53:37.905Z
order: p7777737
board: product
commits:
...
KRMA-019 (verification child; see its comment thread for full context).
...
KRMA-064 conflict) — the `PhotoAsset`/`PhotoAssetID`-based rework of `ImageCollection.Item`
...
`ImageMetadata.swift` conflicts because both branches added `Sendable` conformance and KRMA-018's
...
- Preserve both: KRMA-018's stable `PhotoAssetID` identity and prioritized thumbnail scheduling,
  and KRMA-019's incremental batch publishing, generation-token scan cancellation, and deferred
...
- Full test suite green post-merge, including both `LibraryScanTests` (KRMA-019) and any
  `PhotoAsset`-identity tests from KRMA-018.
