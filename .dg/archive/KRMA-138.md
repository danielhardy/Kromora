...
updated: 2026-09-10T12:53:42.231Z
depends_on:
  - KRMA-129
order: wty9siac
board: product
...
`ImageCollection.thumbnailEntries` (added for KRMA-129) does an `items.firstIndex(where:)` scan per pending slot on every invocation, and it is invoked on each body evaluation of both `LibraryGridView` (twice: empty-state check + row build) and `FilmstripView`. For an N-photo import that is O(N²) `PhotoAssetID` comparisons per update, repeated several times per frame while assets stream in.
...
Found during counterpoint verification of KRMA-129 (commit 2288d66). Not a blocker: measured behaviour is fine at realistic import sizes; this is a scalability/hygiene follow-up.
