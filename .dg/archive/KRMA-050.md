...
updated: 2026-09-10T12:53:37.102Z
depends_on:
  - KRMA-051
  - KRMA-052
  - KRMA-053
...
- KRMA-051 — Define durable export options and format capabilities
- KRMA-052 — Export current or selected photos from originals and saved edits
- KRMA-053 — Add batch progress, cancellation, collision handling, and failure isolation
- KRMA-054 — Add optional Apple Photos delivery after file export is stable (stretch; non-blocking)

## Sequencing

The dependency graph is authoritative. This epic is a tracking issue and depends on every required child ticket; KRMA-054 is a non-blocking stretch ticket. Close the epic only after the outcome above is verified. Start from `.context/initial_concept.md` and preserve existing working LUTzy-derived behavior until its replacement is proven.
...
Integrated Epic 9 implementation is complete across child tickets KRMA-051, KRMA-052, and KRMA-053 (commits e620439, 92e95ec, 749082a). Verified the current/selected original-backed export path, durable format options and validation, full-resolution saved edits, progress/cancellation, collision-safe atomic writes, failure isolation, and bounded batch work. Export-focused suite: 57 passed, 1 expected skip; swift build -c release passed; git diff --check passed; dg validate passed with only known runner-model and low-context warnings. KRMA-054 Apple Photos delivery remains the explicitly non-blocking stretch ticket.
...
**Verdict: PASS — no blockers.** Two non-blocking findings filed as backlog children (KRMA-136, KRMA-137).

Reviewed the integrated implementation (commits e620439, 92e95ec, 749082a) against the epic's MVP outcome, plus each child ticket's own verification record (KRMA-051: pass, KRMA-052: implemented + verified, KRMA-053: pass).

**Correctness**

- Formats: JPEG / 16-bit TIFF / PNG delivered with per-format capability matrices that exactly match the encoder path in `RenderEngine` (`representationFormat` derived from the same `ExportBitDepth`/`ExportAlpha` the matrix validates); JPEG is 8-bit opaque-only with deterministic white-composite flattening. HEIF was consciously left out per KRMA-051's conditional scope — the epic-level MVP bullet remains unmet and is now tracked as **KRMA-137** instead of being silently dropped.
- Metadata: `ExportMetadataPolicy` is modeled, validated, and tested as a value, but the encoder never reads it — every export drops EXIF while the default claims `.preserve`. Real gap for a photographer-facing tool, but honoring it is new product behavior, so filed as **KRMA-136** rather than patched here.
...
- **KRMA-136**: `ExportMetadataPolicy` is a no-op — `RenderEngine`'s encode path never attaches source metadata, so exports lose all EXIF even with `.preserve` (the default).
- **KRMA-137**: HEIF export from the epic's MVP outcome is not delivered (documented out of scope by KRMA-051); tracked so the gap is visible.
...
- dg validate — OK (known runner-model warning only)
Findings:
- KRMA-136: ExportMetadataPolicy is a no-op — the encoder never attaches source metadata, so exports drop EXIF even with .preserve
- KRMA-137: HEIF export from the epic MVP outcome is not delivered (documented out of scope by KRMA-051)
Fixes:
- None
Verification commits:
- None
...
Summary: Counterpoint verification passed: independent review of Epic 9 (e620439, 92e95ec, 749082a) found no blockers; swift test (626, 0 failures), release build, diff-check, dg validate all green. Non-blocking findings filed as KRMA-136 (metadata policy no-op) and KRMA-137 (HEIF MVP gap).
