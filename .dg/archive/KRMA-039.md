...
updated: 2026-09-10T12:53:36.523Z
depends_on:
  - KRMA-036
  - KRMA-037
  - KRMA-038
...
- `dg validate` — OK; only pre-existing warnings (agents.pickup.runner model name, KRMA-035 context completeness), unrelated to this change.
...
- `EffectsAdjustments`/`VignetteAdjustments`/`GrainAdjustments` (pre-existing from KRMA-036/037/038) clamp all fields on both `init` and `didSet`, so the new inspector's free-text `TextField` inputs can't push an out-of-range or non-finite value into the document.
