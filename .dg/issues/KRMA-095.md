...
updated: 2026-09-10T12:53:39.604Z
order: xjve4105
board: product
---
...
**Scope reviewed:** commit b38d394 (fix(KRMA-095): move export format selection into export flow).
...
- Non-blocking UX regression: `ExportCoordinator` used to keep `format` as `@Published` state, so a
  ...
  already `.jpeg`), so not a blocker — filed as KRMA-105 (`verification` label, depends_on
  KRMA-095) for a deliberate follow-up.
