...
updated: 2026-09-10T12:53:46.994Z
depends_on:
  - KRMA-204
order: dyuj0ab1
board: product
...
**Depends on:** KRMA-204
**Epic:** KRMA-181 — see original proposal §40

## 1. Problem

Golden semantic tests (KRMA-204) catch regressions in *facts*, not perceptual regressions in
...
1. A runnable tool that, for every fixture in KRMA-204's corpus: renders the original, runs
   `AutoLightEngine` (KRMA-200) and renders the result, computes a diff, includes the relevant
   mask overlay (reuse KRMA-203's or KRMA-201's overlay-rendering logic, whichever exists), and
   includes `AutoRationale` (KRMA-200).
...
- KRMA-204's fixture corpus, KRMA-200's `AutoLightEngine`/`AutoRationale`, KRMA-201/203's overlay
  logic.
