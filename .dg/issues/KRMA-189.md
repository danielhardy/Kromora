...
updated: 2026-09-10T12:53:45.788Z
depends_on:
  - KRMA-187
  - KRMA-185
order: s2voh9yf
...
"Face" directly in the Masking UI (KRMA-201) — both consume the exact same `.face` `RegionMask`
...
2. Small/background faces (below a documented minimum coverage) are still detected but their
   `RegionMask.confidence`/`coverage` reflect that, so downstream consumers (KRMA-197's ensemble
...
3. **Hard constraint:** no "correct" skin brightness value or target anywhere in this code —
   detection/masking only. Relational judgments belong to KRMA-198 (`RegionRelationships`).
4. Multiple faces produce multiple `RegionMask`s (one `.face` mask per detected face — decide and
   ...
   `.foregroundInstance(Int)`'s pattern in KRMA-184; if `SemanticMaskKind.face` needs an associated
   index for multi-face photos, add it here and update KRMA-184's enum in the same PR).
5. Failure/unsupported throws a typed, catchable error.
6. Coordinates via KRMA-183; Vision revision recorded in `VisionConfiguration`.
...
- If you need to change `SemanticMaskKind` (§2.4 above), that's a legitimate, expected outcome of
  this ticket — KRMA-184 shipped with a best-guess enum, and this is exactly the kind of shape
