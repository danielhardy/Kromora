...
updated: 2026-09-10T12:53:46.523Z
depends_on:
  - KRMA-198
order: oha2vode
board: product
...
**Depends on:** KRMA-198
**Epic:** KRMA-181 — see original proposal §21–23
...
in the whole subsystem and what makes KRMA-200's Auto Light meaningfully better than today's
...
1. Populate `SceneCharacteristics` — `hasFaces`, `hasPeople`, `subjectProminence`,
   ...
   (KRMA-198) and `globalTone` (KRMA-192).
...
- Ship a documented, reasonable-but-tunable formula with named constants — KRMA-207 (Auto tuning)
  exists specifically to refine these against the fixture corpus once it's in place.

## 4. Where to look

- Original proposal §21–23.
- KRMA-198's `RegionRelationships` — primary input.
