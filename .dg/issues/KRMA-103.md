...
updated: 2026-09-10T12:53:40.076Z
order: rxp20km2
board: product
commits:
  - "7795037"
...
Parent: KRMA-096 (verification blocker — unresolved acceptance criteria)
...
KRMA-096 replaced the fixed-square `LazyVGrid` with a justified-row mosaic
...
2. **Unbounded per-update cost.** `mosaicRows` is O(n) over the full filtered set and runs
   ...
   lazy/virtualized behavior from KRMA-021" criterion was meant to rule out.
...
- [ ] Metadata work for aspect ratio (or the mosaic recompute it feeds) is proportional to what's
  ...
  library, so scan-time responsiveness for large libraries matches the KRMA-021 lazy/virtualized
  bar.
