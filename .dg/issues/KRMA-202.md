...
updated: 2026-09-10T12:53:46.764Z
depends_on:
  - KRMA-201
  - KRMA-186
order: rnbt35xc
...
2. Reuses the semantic detection already performed (e.g. the approximate subject/person boundary
   ...
   proposal.
3. Uses `MaskOperations.refine(_:)`'s basic hook (KRMA-186) as a starting point if useful, or
   supersedes it with the real tile-based implementation — reconcile the two in whichever ticket
   lands second.
4. Result written through `MaskStore` (KRMA-185) at `.render` quality, keyed consistently.
...
- This is explicitly scoped as the mask-quality upgrade pipeline only — the paint/brush editing
  ...
  larger effort not covered by this ticket (see KRMA-201's notes).
...
- KRMA-184's `MaskQuality` — the ordering this ticket upgrades along.
- KRMA-185's `MaskStore`, KRMA-186's `MaskOperations`.
