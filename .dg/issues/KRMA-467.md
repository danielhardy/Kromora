...
verification_report:
  ...
  acceptance_criteria:
    - criterion: RenderRequest / RenderEngine funnel semantics and preview quality/resource policies are preserved.
      result: pass
      notes: PreviewCoordinator remains the sole render-admission owner; PreviewAdmissionCoordinator forwards submit/scheduleInteractivePreview/submitCorrective into it unchanged. Verified in Sources/KromoraKit/ViewModels/PreviewAdmissionCoordinator.swift.
    - criterion: Histogram parity and comparison retry behavior are preserved.
      result: pass
      notes: Histogram admission (updateHistogram/cancelHistogram/refreshHistogramGate) and the full comparison-retry cluster (comparisonPreviewScheduledRevision, comparisonPreviewRetriedRevision, comparisonPreviewRetryTask, comparisonPreviewJobID, scheduleOriginalPreview, comparisonPreviewDidFail, cancelComparisonPreview) now live entirely in PreviewAdmissionCoordinator, landed by the KRMA-562 follow-up (commit c8bc39b) and confirmed still present on main. grep of AppViewModel.swift shows only thin forwarders and comparisonBaselineDocument remain on the root.
    - criterion: Collaborator-level fake-based admission/fence tests exist, with AppViewModel integration coverage for navigation, comparison, and histogram behavior.
      result: pass
      notes: "Tests/KromoraKitTests/PreviewAdmissionCoordinatorTests.swift now has all 5 required cases: testIdleAndAdjacentPrefetchJobsHaveIndependentCancellationIDs, testStaleDisplayRevisionAndAssetDropCacheHit, testIdleAdmissionNeverCallsPreviewPublication, testComparisonRetryRunsOncePerComparisonRevision, testROIRequestDoesNotAdoptCanonicalCacheHit. AppViewModel integration coverage (FilmstripNavigationTests, ComparisonModeTests, ThumbnailSwitchLifecycleTests, PreviewDiskCacheTests, HistogramTests, DevelopInspectorTests) remains green."
    - criterion: load(), updateDocument(), applyHistoryDocument(), and shutdown() sequencing remain root-owned.
      result: pass
      notes: Confirmed in AppViewModel.swift; shutdown() calls previewAdmissionCoordinator.shutdown() at the same point the pre-extraction code cancelled debounce/idle/prefetch/comparison-retry work, before previewCoordinator.shutdown()/workScheduler.cancelAllAndWait().
    - criterion: Focused tests, swift build, the relevant fast CI lane, dg validate, and git diff --check pass.
      result: pass
      notes: All five checks re-run independently this session; see checks_run.
  checks_run:
    - "swift build: pass"
    - "swift test --filter 'PreviewAdmissionCoordinatorTests|PreviewPresentationCoordinatorTests|FilmstripNavigationTests|HistogramTests|ComparisonModeTests|PreviewDiskCacheTests': 50/50 pass"
    - "scripts/ci-tests.sh fast: pass (exit 0, 1197/1197)"
  ...
  completed_at: 2026-09-25T04:28:59.065Z
  session: 01MUGGIJZVD8WX1MC0
creation_provenance:
  runner: codex
  model: gpt-5.6-luna
  actor: codex
...
- 2026-09-25T04:28:59.065Z: Verification report
Verdict: PASS
Acceptance criteria:
- [x] RenderRequest / RenderEngine funnel semantics and preview quality/resource policies are preserved. (pass) — PreviewCoordinator remains the sole render-admission owner; PreviewAdmissionCoordinator forwards submit/scheduleInteractivePreview/submitCorrective into it unchanged. Verified in Sources/KromoraKit/ViewModels/PreviewAdmissionCoordinator.swift.
- [x] Histogram parity and comparison retry behavior are preserved. (pass) — Histogram admission (updateHistogram/cancelHistogram/refreshHistogramGate) and the full comparison-retry cluster (comparisonPreviewScheduledRevision, comparisonPreviewRetriedRevision, comparisonPreviewRetryTask, comparisonPreviewJobID, scheduleOriginalPreview, comparisonPreviewDidFail, cancelComparisonPreview) now live entirely in PreviewAdmissionCoordinator, landed by the KRMA-562 follow-up (commit c8bc39b) and confirmed still present on main. grep of AppViewModel.swift shows only thin forwarders and comparisonBaselineDocument remain on the root.
- [x] Collaborator-level fake-based admission/fence tests exist, with AppViewModel integration coverage for navigation, comparison, and histogram behavior. (pass) — Tests/KromoraKitTests/PreviewAdmissionCoordinatorTests.swift now has all 5 required cases: testIdleAndAdjacentPrefetchJobsHaveIndependentCancellationIDs, testStaleDisplayRevisionAndAssetDropCacheHit, testIdleAdmissionNeverCallsPreviewPublication, testComparisonRetryRunsOncePerComparisonRevision, testROIRequestDoesNotAdoptCanonicalCacheHit. AppViewModel integration coverage (FilmstripNavigationTests, ComparisonModeTests, ThumbnailSwitchLifecycleTests, PreviewDiskCacheTests, HistogramTests, DevelopInspectorTests) remains green.
- [x] load(), updateDocument(), applyHistoryDocument(), and shutdown() sequencing remain root-owned. (pass) — Confirmed in AppViewModel.swift; shutdown() calls previewAdmissionCoordinator.shutdown() at the same point the pre-extraction code cancelled debounce/idle/prefetch/comparison-retry work, before previewCoordinator.shutdown()/workScheduler.cancelAllAndWait().
- [x] Focused tests, swift build, the relevant fast CI lane, dg validate, and git diff --check pass. (pass) — All five checks re-run independently this session; see checks_run.
Checks run:
- swift build: pass
- swift test --filter 'PreviewAdmissionCoordinatorTests|PreviewPresentationCoordinatorTests|FilmstripNavigationTests|HistogramTests|ComparisonModeTests|PreviewDiskCacheTests': 50/50 pass
- scripts/ci-tests.sh fast: pass (exit 0, 1197/1197)
...
Actor: claude
Resolved model: sonnet
Pickup session: 01MUGGIJZVD8WX1MC0
Summary: Re-verified after KRMA-562 landed the missing comparison-retry extraction and the four missing fake-based tests: comparison-retry cluster is fully owned by PreviewAdmissionCoordinator, all 5 required fence tests exist and pass, docs/APP_ARCHITECTURE.md is accurate, and all required checks (build, focused tests, ci-tests.sh fast, dg validate, git diff --check) pass cleanly.
