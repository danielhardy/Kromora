...
updated: 2026-09-10T12:53:38.085Z
depends_on:
  - KRMA-069
  - KRMA-070
  - KRMA-071
...
Light tone curve. Performance takes priority over implementation size. KRMA-067 established
...
- [ ] The real-hardware latency gates in KRMA-072 pass for representative standard and RAW images.
- [ ] Export and settled preview remain pixel-consistent with the final edit document.

## Implementation notes

The focused implementation work is split into KRMA-069 (render and presentation), KRMA-070 (RAW
Develop), KRMA-071 (tone-curve allocation/upload), and KRMA-072 (end-to-end measurement). Preserve
...
### Scope reviewed
Umbrella KRMA-068 and dependencies KRMA-069/070/071/072 (plus verification follow-ups
...
- `dg validate` — OK; only pre-existing runner-model + KRMA-076 low-context warnings.
...
- Settled upgrade is in-place, no blank/fade/repaint: settled phase publishes the same GPU surface
  and skips redundant CGImage raster (KRMA-075); test `testSettledGPUPublicationDoesNotRasterizeASecondImage`.
...
- Live-edit telemetry (KRMA-072): separate signposts for PointerInput/RenderStart/End/
  GPUComplete/DrawablePresented/StaleRevision keyed by revision; quantile/FPS/gap/coalesce report.

### Non-blocking findings
- **KRMA-079 filed (backlog, `verification`, parent KRMA-068):** `PreviewSurfaceView.updateNSView`
...
### Existing governance
KRMA-074 (measured tone-curve win) and KRMA-075 (redundant settled raster) are done and confirmed
here; KRMA-076 (GPU surface uses WorkingSpace.current, currently inert) remains open low-priority,
as filed.
...
- 2026-09-01T02:28:19.020Z: Counterpoint verification PASS after human review. 429 tests pass, debug+release builds clean, tone-curve measured 18x speedup, live-edit p95 7.0ms (fake) / instrumentation infra confirmed. KRMA-079 filed (non-blocking: telemetry measures submit not present time).
