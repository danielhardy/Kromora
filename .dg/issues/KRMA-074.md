...
updated: 2026-09-10T12:53:38.421Z
depends_on:
  - KRMA-071
order: 4r2lb8bx
board: product
...
Parent: KRMA-071 (verification finding, non-blocking)

## Context

KRMA-071 replaced the per-render 64³ `CIColorCube` tone-curve construction with an
actor-confined 256×1 texture + Core Image kernel (commit c459816, verification fix e6617d8).
The acceptance criteria for KRMA-071 called for measured benchmarks — graph construction cost,
...
Checked commit 41c99cf against the acceptance criteria and the KRMA-071 finding it addresses.

- **Benchmark correctness**: `testMeasureToneCurveDragCost` reconstructs the pre-KRMA-071 64³
