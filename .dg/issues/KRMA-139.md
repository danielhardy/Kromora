...
updated: 2026-09-10T12:53:42.298Z
depends_on:
  - KRMA-129
order: 9cyk5rbh
board: product
...
Two edge cases in `ImageCollection.thumbnailEntries` (added for KRMA-129) are currently implicit. Decide the intended behaviour and cover it with tests:
...
Found during counterpoint verification of KRMA-129 (commit 2288d66). Not a blocker: neither case is reachable through the current `PHPhotosPicker` flow (selection count equals enumerated ordinals, and filters are not user-changeable mid-import in the shipped UI).
