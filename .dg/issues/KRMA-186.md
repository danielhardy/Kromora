...
updated: 2026-09-10T12:53:45.558Z
depends_on:
  - KRMA-184
order: pmr2lb4a
board: product
...
Both Auto's regional statistics (KRMA-193) and the eventual Masking UI (KRMA-201) need to combine
...
1. `enum MaskOperations` (or a small set of free functions/methods on `RegionMask`) providing:
   ...
   - `refine(_:) -> RegionMask` (placeholder hook for KRMA-202's higher-quality refinement —
...
2. Operations compose: `subtract(faceMask, from: personMask)` should produce a valid `RegionMask`
   ...
   precise input).
3. Every operation works purely on `RegionMask` + whatever pixel access `MaskStore` (KRMA-185)
...
- KRMA-184 — `RegionMask`, `SemanticMaskKind`, `MaskQuality`.
- KRMA-185 — `MaskStore`, where operation results get cached.
