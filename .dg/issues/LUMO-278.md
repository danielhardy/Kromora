...
updated: 2026-09-08T19:16:34.043Z
parent: LUMO-261
depends_on:
  - LUMO-261
order: a0
...
Found while verifying LUMO-261 (relink assetID collisions). Repro test:
`EditDocumentStoreTests.testRelinkCollisionRollsBackOnPersistFailureSoRetrySucceeds` in
`Tests/LumoKitTests/EditDocumentStoreTests.swift` (added on LUMO-261's verification commit).
...
This predates LUMO-261 — the same `restoreActionableStatus` call already existed on the
non-collision, direct-hit relink branch — but LUMO-261 is the first place a retry-after-failure
...
`Sources/LumoKit/Models/EditDocumentStore.swift`. Not a blocker for LUMO-261: data integrity and
