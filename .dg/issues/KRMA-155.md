...
updated: 2026-09-10T12:53:43.377Z
depends_on:
  - KRMA-160
  - KRMA-161
order: z4w97rs4
...
**Blocker 2 — main does not build from a clean checkout** ([[KRMA-161]], pre-existing,
...
as far back as `b989c0b` (KRMA-150). The "swift test — 700 passed... swift build -c
...
**Action taken**: created urgent child tickets KRMA-160 (commit the Settings entry
point) and KRMA-161 (fix the clean-checkout build break), both parented to and
blocking this issue via `depends_on`. No source changes made — nothing here is a
safe, localized fix within KRMA-155's scope; committing the dangling
