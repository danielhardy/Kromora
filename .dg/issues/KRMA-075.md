...
updated: 2026-09-10T12:53:38.473Z
order: gzpmr2im
board: product
commits:
  - aa08b38
...
Parent: KRMA-073 (verification follow-up — non-blocking)

## Context

KRMA-073 added a persistent GPU presentation surface (`PreviewSurface`/`PreviewSurfaceView`) that
...
release, slider commit), not a correctness break — KRMA-073 verification confirmed the GPU surface
...
Verification report (pi, KRMA-075) — PASS
...
- dg validate — OK (2 unrelated known warnings: codex runner model, KRMA-076 context completeness)
...
- KRMA-078 (backlog child, verification label, parent KRMA-075): the main-preview NSImage fallback path (PreviewView fallback branches, AppViewModel previewNSImage/originalPreviewNSImage publishing) is permanently dead once a surface has ever presented; consider removing or re-scoping to a documented seam.
- Observation only (pre-existing from KRMA-073, not introduced here): if makeCIImage ever returned nil while makeCGImage succeeded, publishPreview would leave a stale surface frame visible while setting the NSImage — a non-issue on the real engine (both go through buildImage), but worth a comment if the seam survives.

Verification commits: none (review only; no source changes made).

## Agent log

- 2026-09-01T02:07:29.160Z: Verified: settled GPU previews no longer rasterize a redundant CGImage; tests 429/24-skip green, release build clean, dg validate OK. Non-blocking follow-up tracked as KRMA-078.
