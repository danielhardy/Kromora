...
updated: 2026-09-10T12:53:45.482Z
depends_on:
  - KRMA-184
order: mr2lb892
board: product
...
`RegionMask` (KRMA-184) references its pixel data via `RegionMaskReference` rather than embedding
it, so there must be a store: something that persists mask pixel data, keyed consistently with
`PhotoAnalysis`'s own cache (KRMA-196), and versioned the same way. This is the mask-specific twin
...
1. A persistent, concurrency-safe store (actor, or otherwise `Sendable`-safe) — `MaskStore` —
   ...
   ```
2. Cache key includes asset identity, source fingerprint (same notion KRMA-196 will use for
   ...
   provider/Vision-configuration version (from KRMA-187).
...
- Cancelled writes must not corrupt the store — mirror the cancellation-safety discipline already
  established for per-photo persistence flushes (see KRMA-175/176's precedent, cited from the
...
- KRMA-184 — `RegionMaskReference`, `MaskCacheKey`, `MaskQuality` this store operates on.
