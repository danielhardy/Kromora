...
updated: 2026-09-10T12:53:38.370Z
order: udtnwjg7
board: product
---

Parent: KRMA-069 (verification blocker — unresolved acceptance criteria)

## Context

KRMA-069's objective was to replace per-frame image-object publication with a frame-paced GPU
...
## Unresolved acceptance criteria (from KRMA-069)
...
Design and implement the actual presentation-surface swap called for in KRMA-069's acceptance
...
directly during KRMA-069 verification.
...
Reviewed commit 14a1ca3 against the three unresolved KRMA-069 acceptance criteria and found the
...
### Confirmed already-working (per KRMA-069 AC, re-verified after the fixes)
...
- **KRMA-075** (medium): settled-frame publish still does a redundant second GPU render
...
- **KRMA-076** (low): `PreviewSurfaceView` renders with a hardcoded `WorkingSpace.current.cgColorSpace`
...
Verification commit: f3bb800 (`KRMA-073 fix GPU preview surface: present drawable, don't drop
