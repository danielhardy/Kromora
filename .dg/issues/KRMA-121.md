---
id: KRMA-121
title: Capture representative hardware evidence for KRMA-107's warm-transform/settle latency targets
type: task
status: done
priority: medium
labels:
...
updated: 2026-09-10T12:53:41.178Z
order: yez4w92b
board: product
---
...
Capture a representative Release-build hardware trace of the pipeline landed in KRMA-107 (commit
3195e55, off-main-actor completed-texture rendering + presentation-only transform pass), and record
whether it meets KRMA-107's measured acceptance criteria: p95 input-to-present ≤33 ms for warm
...
Independent counterpoint verification of KRMA-107 (2026-09-02) found the implementation,
...
This mirrors the KRMA-118/KRMA-119 precedent: measurement requires a logged-in Mac with a real
display and `scripts/run-lumo-118-capture.sh` (or equivalent), which is not available from this
verification session. Per that precedent, this capture gap does not block KRMA-107 from completing
...
- A logged-in Mac with a real display, Release build, and `xctrace` (Points of Interest + Metal
  System Trace), same setup as KRMA-118/119.
...
- Compare against the ≤33 ms p95 / ≤200 ms settle targets in KRMA-107's acceptance criteria and
  record the result (met, not met, or partially met) in the capture matrix / summary doc.

## Scope boundaries

Do not fabricate or estimate numbers from the pre-3195e55 KRMA-118 capture — it measured a
different code path. Parent: KRMA-107.
...
**Scope:** Independent verification of the KRMA-121 hardware capture (commit 59a9fc5) against the
issue's acceptance criteria and the KRMA-107 numeric targets.
...
- `swift test` (full suite): 594 tests, 1 failure — `RAWCapabilitiesTests.testProbingARealRAWReportsItsDecodersSeeds`
  (host-dependent lens-correction default; file untouched by this commit, unrelated to KRMA-121).
...
- Reviewed the `MetalPresentationBenchmark.swift` diff: the warm-sample path now renders through
  ...
  transforms for the timed samples, correctly isolating the KRMA-107 boundary; the settle path uses
...
**Result:** Both KRMA-107 targets are met on this representative run — warm p95 input-to-present
