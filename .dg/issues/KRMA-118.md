---
id: KRMA-118
title: Archive representative hardware capture data for KRMA-114
type: task
status: done
priority: urgent
labels:
...
updated: 2026-09-10T12:53:40.994Z
order: hf9i56jp
board: product
---
...
UI interaction is required. Parent: KRMA-114, which no longer blocks on completion of this capture
follow-up. KRMA-119 was a manual-capture fallback and is now closed as obsolete.
...
No maintainability/security/performance issues found in the surrounding infrastructure (KRMA-117's benchmarks and matrix doc) during this pass — the gap is exclusively the archived-data rows this ticket exists to close, and closing it requires a human with a Release build, a real display, and licensed 24 MP / 40-60 MP RAW files.

Action taken per verification policy for an unresolved blocker: filed KRMA-119 (priority urgent, label verification, scoped to provisioning/running the human capture session) as a dependency of this ticket (KRMA-118 depends_on KRMA-119). No source or doc changes made — the pending-state matrix record is accurate and should not be altered until real captures exist. Returning KRMA-118 to review; do not re-dispatch to an implementation agent until KRMA-119's human capture session is scheduled/completed.
