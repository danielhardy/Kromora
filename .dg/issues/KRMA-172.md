...
verification_report:
  ...
  checks_run:
    ...
    - manual read of RenderEngine.swift/BoundedCache.swift/RenderEngineResources.swift diff and docs/ENGINEERING_GUIDE.md
  findings:
...
updated: 2026-09-10T12:53:44.504Z
order: cdtnwjj2
board: product
---
...
Implemented in 35c9da3. Render intermediate admission now estimates RGBA-half CPU plus GPU working-set bytes with saturating arithmetic before allocation; above-budget processing prefixes stay fused/non-cached, and above-budget RAW outputs remain request-local instead of entering the settled cache. Added docs/ENGINEERING_GUIDE.md and standard/RAW materialization-storm regressions (RAW test follows the existing opt-in local-fixture policy). Verified: swift test (740 passed, 33 skipped), focused above-budget tests (1 standard passed, 1 RAW skipped without fixture), swift build -c release, git diff --check, dg validate. Repository-wide format script still reports pre-existing violations in already-modified files; direct strict lint passes for the newly touched resource/cache files.
