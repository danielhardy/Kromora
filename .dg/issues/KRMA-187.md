...
updated: 2026-09-10T12:53:45.634Z
depends_on:
  - KRMA-183
  - KRMA-184
order: qs82axv6
...
boundary as a concrete `SemanticMaskProviding` (KRMA-184) conformer — `VisionSemanticMaskProvider`
— **without** implementing any specific request; that's KRMA-188/189/190/191 (one per semantic
...
2. Internally, route `mask(for:image:quality:)` to per-kind Vision requests — this ticket can ship
   with every kind unimplemented (`throw`/`unsupported`) and let KRMA-188/189/190/191 each fill in
...
- Coordinate conversion was already built in KRMA-183 — call it, don't reimplement it.
