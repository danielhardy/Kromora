...
updated: 2026-09-10T12:53:46.601Z
depends_on:
  - KRMA-199
order: omgpcdpr
board: product
...
(KRMA-194/197/198/199). **Auto is the first, but not the only, consumer of the mask foundation** —
this ticket must consume masks the same way the Masking UI (KRMA-201) will, through the shared
...
level (`.analysis`), which the shared foundation already supports by design (KRMA-184).
...
6. Preserves artistic intent per `SceneCharacteristics` (KRMA-199): high-key stays high-key,
...
8. **Integration**: `AppViewModel.runAutoAdjustment()` gains a path that, when `PhotoAnalysis` is
   available (from KRMA-195's coordinator, `.standard` level or better) and
...
- **Do not build a second mask-consumption path for Auto.** If you find yourself writing code that
  ...
  (KRMA-193), stop — that's the bug this whole restructure exists to prevent.
...
- KRMA-193's `MaskedToneAnalyzer`, KRMA-184's `SemanticMaskProviding` — the only sanctioned way to
