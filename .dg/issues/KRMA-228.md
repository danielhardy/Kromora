---
id: KRMA-228
title: "Commit KRMA-218/219 local-mask model work: HEAD does not build without untracked files"
type: bug
status: done
priority: urgent
creation_provenance:
...
updated: 2026-09-10T12:53:48.784Z
order: zf9i56gu
board: product
---
...
Land the durable local-mask model as an actual git commit so KRMA-218 and KRMA-219 are reproducible
from a clean checkout.

## Problem

KRMA-219 (`58a690a KRMA-219 render local adjustments through soft masks`) is committed, but its
...
- `Sources/LumoKit/Models/MaskInteractionState.swift` and `Tests/LumoKitTests/LocalMaskTests.swift`
  (KRMA-218's stated deliverables) are also untracked.
...
KRMA-218 is marked `status: done` with `verification_report.verdict: pass`, but that verification
recorded empty `checks_run`/`findings`/`verification_commits` and did not catch that no commit for
KRMA-218 exists at all.
...
- [ ] `LocalMaskModels.swift`, `MaskInteractionState.swift`, `Tests/LumoKitTests/LocalMaskTests.swift`,
      ...
      migration) land as a real commit on `main`, scoped to KRMA-218's stated deliverables only.
...
- [ ] KRMA-218's issue record reflects the actual commit sha in a follow-up verification note.

## Context

The working tree currently mixes uncommitted changes for several other in-flight tickets
(KRMA-215/217/220-227 among the modified/untracked files in `git status`). Whoever picks this up
should commit only the KRMA-218-scoped files, not the unrelated changes, per the repo's "one issue
per commit" convention in `.dg/AGENTS.md`.


### Comment — codex @ 2026-09-05T04:03:10.033Z

Implemented in commit fe95f16. The KRMA-218 model and document integration are now tracked, so KRMA-219 no longer depends on untracked source files. Remaining in-progress capture work is being preserved separately from the active checkout.
