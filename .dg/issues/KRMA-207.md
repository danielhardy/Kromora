...
updated: 2026-09-10T12:53:47.167Z
depends_on:
  - KRMA-200
  - KRMA-204
  - KRMA-205
...
**Depends on:** KRMA-200, KRMA-204, KRMA-205
**Epic:** KRMA-181 — see original proposal §41, §44–46

## 1. Problem

`AutoLightEngine` (KRMA-200) and `SceneCharacteristicsAnalyzer` (KRMA-199) ship with reasonable-
but-unvalidated constants. This ticket is the deliberate tuning pass against the fixture corpus
(KRMA-204) using the visual regression harness (KRMA-205).

## 2. Requirement (acceptance criteria)

1. Run the harness against the full corpus and record a baseline report before changes.
2. Adjust named constants in KRMA-199/200 based on the corpus report — every change justified by
...
- If the corpus is still small at this point (KRMA-204 scopes it to dozens of synthetic fixtures
...
- KRMA-200's `AutoLightEngine`, KRMA-199's `SceneCharacteristicsAnalyzer` — what's tuned.
- KRMA-204's corpus, KRMA-205's harness — the tools this ticket runs.
...
- [x] Adjust named constants in KRMA-199/200 based on corpus report, each justified by before/after comparison (pass)
...
Findings:
- Generated report artifacts/photo-intelligence/*.html were untracked but not gitignored -- fixed directly (localized, non-product .gitignore change, commit f26eb36).
- Pre-existing bug (predates this ticket, commit 7e990f07): AutoLightSceneSignals.exposure's rationale string is missing string interpolation around format(tone.p50) at AutoLightEngine.swift:325 -- the exposure rationale text always shows the literal text 'format(tone.p50)' instead of the formatted median. Out of scope for this tuning ticket; filed as KRMA-208 (label verification, parent KRMA-207).
Fixes:
- Added artifacts/ to .gitignore for generated photo-intelligence visual regression reports (commit f26eb36)
Verification commits:
- f26eb36
...
Summary: Independent verification pass: reviewed diff, re-ran targeted tests and build, confirmed corpus tuning claims and golden-range test coverage. Filed KRMA-208 for a pre-existing (out-of-scope) rationale-string bug found during review; fixed a small .gitignore gap for generated report artifacts.
